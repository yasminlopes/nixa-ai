...
const MIN_LEXICAL_RELEVANCE = 0.1;
...
async function getLocalStore(): Promise<LocalStore> {
  ...
  if (_localStore.chunks.length > 0 && !_localStore.embeddingProvider) {
    ...
    _localStore.dims = _localStore.chunks.find((chunk) => chunk.embedding?.length)?.embedding
      .length;
  }
...
function queryAllowsThirdParty(query: string): boolean {
  ...
  return (
    ...
    queryLower.includes('microsoft') ||
    queryLower.includes('azure') ||
    queryLower.includes('slack') ||
    queryLower.includes('jira') ||
    queryLower.includes('atlassian')
  );
}
...
function isLikelyThirdPartyNoise(doc: DocChunk): boolean {
  ...
  const hasNoiseContent = THIRD_PARTY_NOISE_CONTENT_PATTERNS.some(
    (pattern) => head.includes(pattern) || title.includes(pattern),
  );
...
function looksTechnicalQuery(query: string): boolean {
  const queryLower = query.toLowerCase();
  return ['api', 'sdk', 'auth', 'token', 'webhook', 'endpoint', 'código', 'codigo', 'code'].some(
...
async function embedChunksWithConcurrencyControl(
  ...
  const results: {
    embedding: number[];
    model: string;
    chunk: Omit<DocChunk, 'id' | 'embedding'>;
  }[] = [];
  const queue = [...chunks];
  ...
  const processChunk = async (chunk: Omit<DocChunk, 'id' | 'embedding'>) => {
    ...
    try {
      ...
      if (String((err as { message?: string })?.message ?? '') !== 'EMBEDDINGS_UNAVAILABLE')
        throw err;
...
export async function addDocChunks(
  ...
  const embeddedChunks = await embedChunksWithConcurrencyControl(
    chunks,
    provider,
    apiKey,
    onWarning,
...
export async function searchSimilarDocs(params: {
  ...
  try {
    const { embedding: queryEmbedding } = await getEmbedding(
      semanticQuery,
      embeddingProvider,
      params.apiKey ?? '',
    );
    ...
    if (store.dims && queryEmbedding.length !== store.dims) {
      console.warn(
        `[VECTORSTORE] dim mismatch (query=${queryEmbedding.length}, índice=${store.dims}) — reindexe. Fallback léxico.`,
      );
    ...
    console.error(
      '[VECTORSTORE] busca semântica falhou, fallback léxico:',
...
export async function getStoreStats(): Promise<{ count: number; sources: string[] }> {
  ...
  const sources = Array.from(
    new Set(store.chunks.map((chunk) => chunk.metadata.source).filter(Boolean)),
  );
