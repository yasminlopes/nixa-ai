...
export function classifyPage(url: string, title: string): PageType {
  ...
  if (
    signal.includes('release') ||
    signal.includes('whatsnew') ||
    signal.includes('what-s-new') ||
    signal.includes('changelog')
  ) {
    return 'release';
  }
  if (
    signal.includes('troubleshoot') ||
    signal.includes('known issue') ||
    signal.includes('resolve issue') ||
    signal.includes('common problem')
  ) {
    return 'troubleshooting';
  }
  if (
    signal.includes('/api/') ||
    signal.includes('api reference') ||
    signal.includes('endpoint') ||
    signal.includes('rest api')
  ) {
    return 'api';
  }
  if (
    signal.includes('reference') ||
    signal.includes('schema') ||
    signal.includes('sdk') ||
    signal.includes('modules.html')
  ) {
    return 'reference';
  }
  if (
    signal.includes('config') ||
    signal.includes('setup') ||
    signal.includes('install') ||
    signal.includes('provision') ||
    signal.includes('administra')
  ) {
  ...
  if (
    signal.includes('guide') ||
    signal.includes('gettingstarted') ||
    signal.includes('tutorial') ||
    signal.includes('getting started')
  ) {
    return 'guide';
  }
  return 'other';
