...
export function useChat({ conversationId, onConversationSaved }: ChatViewProps) {
  ...
  function saveConversation(msgs: MessageType[]) {
    const title =
      msgs.find((message) => message.role === 'user')?.content.slice(0, 60) ?? 'Nova conversa';
    const stored = localStorage.getItem('nixa-conversations');
    ...
    if (existing) {
      ...
      updated = convs.map((conversation) =>
        conversation.id === savedConv.id ? savedConv : conversation,
      );
    } else {
      savedConv = {
    ...
    try {
      ...
      for await (const chunk of stream) {
        finalMessages = newMessages.map((message) =>
          message.id === aiMsg.id
            ? { ...message, content: chunk.content, sources: chunk.sources }
            : message,
        );
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        setMessages((prev) =>
          prev.map((message) =>
            message.id === aiMsg.id
              ? { ...message, content: message.content || '__interrupted__' }
              : message,
          ),
        );
      } else {
        setMessages((prev) =>
          prev.map((message) =>
            message.id === aiMsg.id
              ? {
                  ...message,
                  content: `❌ Erro ao gerar resposta. Detalhe: ${error instanceof Error ? error.message : String(error)}`,
                }
              : message,
          ),
        );
