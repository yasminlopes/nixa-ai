...
export function IndexTab({ onRunningChange, onClose }: IndexTabProps) {
  ...
  const hasWarnings =
    running &&
    logs.some(
      (log) => log.includes('⚠️') || log.includes('rate limit') || log.includes('indisponível'),
    );
  ...
  async function startIndexing() {
    ...
    try {
      ...
      if (sawFatal) {
        throw new Error(
          'A indexação falhou no servidor. Confira os logs abaixo e tente novamente.',
        );
      }
      if (!sawComplete) {
      ...
      try {
        const stats = await fetch('/api/index-docs').then((response) =>
      ...
      setErrorMessage(
        isAbort
          ...
          : error instanceof Error
            ? error.message
            : 'Erro inesperado durante a indexação.',
      );
      setStatus('error');
    } finally {
    ...
    <div>
      ...
      {status === 'idle' && logs.length === 0 ? (
        <div className={styles.card}>
          ...
          <Callout icon={AlertTriangle} tone="neutral">
            Pode levar alguns minutos e consumir requests do provedor de embeddings. URLs já
            indexadas recentemente são puladas automaticamente.
          </Callout>
          ...
          <label className={styles.forceRow}>
            <button
              ...
              style={{
                background: forceReindex ? 'var(--color-accent)' : 'var(--color-border-strong)',
              }}
            >
              <span
        ...
        <>
          <div ref={logsRef} className={styles.terminal} aria-live="polite">
            {logs.map((log, i) => (
              <p key={i} className={logClass(log)}>
                {log}
              </p>
            ))}
            {running && <p className={styles.processing}>▋ processando...</p>}
          </div>
          ...
          {hasWarnings && (
            <Callout
              icon={AlertTriangle}
              tone="accent"
              title="Taxa de requisição atingida"
              className={styles.spacedTop}
            >
      ...
      {status === 'success' && (
        <div className={styles.doneCard} role="status">
          ...
          <div className={styles.doneActions}>
            ...
            {autoCloseIn !== null && (
              <span className={styles.reloadCountdown}>fechando em {autoCloseIn}s…</span>
            )}
          </div>
        </div>
      )}
      ...
      {status === 'error' && (
        <div className={styles.errorCard} role="alert">
          ...
          {errorMessage && <p className={styles.errorReason}>{errorMessage}</p>}
          <div className={styles.doneActions}>
