...
import { retrieve, isLikelyDomainQuestion } from './retrieval.service'
import { buildPrompt } from './prompt.service'
import { generate, extractRetryDelaySeconds } from './providers'
...
export type AskNixaResult =
  | { kind: 'message'; message: string; sources: ChatSource[]; status?: number; retryAfterSeconds?: number }
...
async function* resilientStream(
  ...
  try {
    ...
    console.error(`[AI] ${providerLabel} stream error:`, error instanceof Error ? error.message : error)
...
export async function askNixa(params: AskNixaParams): Promise<AskNixaResult> {
  ...
  const sources = uniqueSources(documents.map(doc => ({
  ...
  if (result.rateLimitError) {
    ...
    return {
      ...
      sources: uniqueSources(documents.slice(0, 3).map(doc => ({
        title: doc.metadata.title,
        url: doc.metadata.url,
      }))),
