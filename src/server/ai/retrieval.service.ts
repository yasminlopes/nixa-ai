...
export function isInconclusiveContext(params: {
  ...
  const strongCount = params.relevantDocs.filter(doc => strongTypes.has(doc.metadata.pageType ?? '')).length
 ...
 */
export async function retrieve(params: { question: string; k?: number; embeddingApiKey?: string }): Promise<RetrievalResult> {
